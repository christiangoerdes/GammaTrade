 ...
 */
Stock::Stock(const std::string& name, const double start, const double mean, const double stddev, unsigned seed) 
    : _name(name), _start(start), _mean(mean), _stddev(stddev), generator(seed), distribution(0.0, 1.0) {
 ...
 */
Stock::Stock() : _start(0.0), _mean(0.0), _stddev(1.0), distribution(0.0, 1.0) {}

/**
 * @brief Updates the stock's price based on a Brownian Motion model.
 */
void Stock::update(const int timespan) {
    ...
    if (_price_history.size() > timespan) {
